  ...
  body: Joi.object({
    ...
    timesPerDay: Joi.number()
      ...
      .messages({
        ...
        "any.unknown":
          "Times per day is not allowed when frequency is not DAILY",
      }),
    daysOfWeek: Joi.array()
      ...
      .messages({
        ...
        "any.unknown":
          "Days of week is not allowed when frequency is not WEEKLY",
      }),
...
export const confirmAddMedicineSchema = {
  body: Joi.object({
    ...
    reminders: Joi.array()
      ...
      hasInteractions: Joi.boolean().optional().messages({
        "boolean.base": "Has interactions must be a boolean",
        "any.unknown": "Has interactions is not allowed",
      })
  }),
...
export const updateMedicineSchema = {
  ...
  body: Joi.object({
    medicineName: Joi.string().trim().min(1).max(100).optional().messages({
      "string.base": "Medicine name must be a string",
      "string.empty": "Medicine name cannot be empty",
      "string.min": "Medicine name must be at least 1 character long",
      "string.max": "Medicine name must not exceed 100 characters",
    ...
    startHour: Joi.number().min(0).max(23).optional().messages({
      "number.base": "Start hour must be a number",
      "number.min": "Start hour must be at least 0",
      "number.max": "Start hour must not exceed 23",
    }),
    startDateTime: generalRules.startDate.messages({
      "string.isoDate":
        "Start date-time must be a valid ISO date string (e.g., 2025-04-15)",
      "any.required": "Start date-time is required",
      "any.unknown": "Start date-time is not allowed",
    }),
    endDateTime: generalRules.endDate.messages({
      "string.isoDate":
        "End date-time must be a valid ISO date string (e.g., 2025-04-15T20:00:00Z)",
      "date.greater": "End date-time must be greater than start date-time",
      "any.required":
