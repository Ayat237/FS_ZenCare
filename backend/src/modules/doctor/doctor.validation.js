import Joi from "joi";
import { generalRules, Gender, possibleRoles } from "../../utils/index.js";
import { Specialties } from "../../utils/enums.utils.js";

const addressSchema = Joi.object({
  street: Joi.string().min(3).max(100).required(),
  city: Joi.string().min(2).max(50).required(),
  country: Joi.string().min(2).max(50).default("Egypt"),
  buildingNumber: Joi.number().integer().min(1).optional(),
  buildingName: Joi.string().min(2).max(100).optional(),
...
export const doctorRegisterSchema = {
  body: Joi.object({
    ...
    role: Joi.array().items(Joi.string().valid(possibleRoles.DOCTOR)).required(),
    gender: Joi.string().valid(Gender.MALE, Gender.FEMALE, Gender.OTHER).required(),
    specialty: Joi.string().valid(...Object.values(Specialties)).required(),
    yearsOfExperience: Joi.number().min(0).max(60).required(),
    education: Joi.array().items(
      Joi.object({
        degree: Joi.string().required(),
        institution: Joi.string().required(),
        graduationYear: Joi.number().min(1900).max(new Date().getFullYear()).required(),
      })
    ).min(1).required(),
    ...
    // verificationId: Joi.any().optional(), //   file validation handled by multer
...
export const updateDoctorProfileSchema = {
  body: Joi.object({
    ...
    specialty: Joi.string().valid(...Object.values(Specialties)).optional(),
