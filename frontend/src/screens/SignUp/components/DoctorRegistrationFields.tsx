...
const DoctorRegistrationFields: React.FC<DoctorRegistrationFieldsProps> = ({
  ...
  const handleClinicBranchChange = (
    index: number,
    field: string,
    value: any
  ) => {
  ...
  const handleLocationConfirmed = async (location: {
    ...
    try {
      console.log("Location confirmed in DoctorRegistrationFields:", location);

      // Explicitly fetch the display name from OpenStreetMap API
      ...
      if (response.ok) {
        ...
        // Use the display_name from the API
        if (data.display_name) {
          displayName = data.display_name;
          console.log("Using display name from API:", displayName);
        }
      }

      // Fallback if API call fails or doesn't return a display_name
      if (!displayName) {
        displayName = `Location at ${location.latitude.toFixed(
          4
        )}, ${location.longitude.toFixed(4)}`;
        console.log("Using fallback display name:", displayName);
      }

      // Update the branches with the new address
      const updatedBranches = [...formData.clinicBranches];
      ...
      // Even if there's an error, ensure we update with the basic info
      const updatedBranches = [...formData.clinicBranches];
      updatedBranches[currentBranchIndex] = {
        ...updatedBranches[currentBranchIndex],
        address: {
    ...
    <View style={styles.container}>
        ...
        <View style={styles.dropdownContainer}>
          ...
          >
            <Text
              style={[
                styles.dropdownText,
                !formData.specialty && styles.placeholderText,
              ]}
            >
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    style={[
                      ...
                      formData.specialty === item && styles.selectedSpecialty,
                    ]}
      ...
      <View style={styles.fieldContainer}>
        ...
        {errors.hospitalAffiliations && (
          <Text style={styles.errorText}>{errors.hospitalAffiliations}</Text>
        )}
      </View>
      ...
      <View style={styles.fieldContainer}>
        ...
        {formData.clinicBranches.map((branch, index) => (
          <View key={index} style={styles.branchContainer}>
            ...
            branch.address.coordinates.longitude !== 0 ? (
              <View style={styles.addressPreview}>
                ...
                <Text style={styles.coordsText}>
                  Lat: {branch.address.coordinates.latitude.toFixed(4)}, Lon:{" "}
                  {branch.address.coordinates.longitude.toFixed(4)}
                </Text>
              </View>
            ) : (
            ...
            <InputField
              ...
              onChangeText={(value) =>
                handleClinicBranchChange(index, "phoneNumber", value)
              }
        ...
        {errors.clinicBranches && (
          <Text style={styles.errorText}>{errors.clinicBranches}</Text>
        )}
      </View>
