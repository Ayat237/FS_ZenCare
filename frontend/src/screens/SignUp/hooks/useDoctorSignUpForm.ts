...
export const useDoctorSignUpForm = () => {
  const [doctorFormData, setDoctorFormData] = useState<DoctorFormData>(INITIAL_DOCTOR_FORM_DATA);
  ...
  const handleDoctorFormChange = useCallback((newData: Partial<DoctorFormData>) => {
  ...
  const validateDoctorForm = (): boolean => {
    ...
    const hasIncompleteHospital = doctorFormData.hospitalAffiliations.some(hospital => !hospital.name);
    if (doctorFormData.hospitalAffiliations.length === 0 || hasIncompleteHospital) {
      errors.hospitalAffiliations = 'At least one hospital affiliation is required';
    }

    // Validate clinic branches
    const hasIncompleteClinic = doctorFormData.clinicBranches.some(
      branch => {
        // Check if coordinates are valid (not 0,0)
        const hasValidCoordinates = branch.address.coordinates && 
        ...
        if (hasValidCoordinates) {
          ...
          return !branch.address.street || !branch.address.city || !branch.address.country || !branch.phoneNumber;
        }
      }
    );
